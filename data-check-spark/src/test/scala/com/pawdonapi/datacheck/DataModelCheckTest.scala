...
class DataModelCheckTest extends AnyFunSuite with SparkSessionTestWrapper {
  test("uniqueness") {
    val order_details = spark.sql(
      """
          values
          (1, 'a', 'x'),
          (1, 'b', 'x'),
    ...
    val result = DataModelCheck.uniqueness(
      src = order_details,
      keys = Seq("order_id", "item_id")
    )
  ...
  test("relationship") {
    ...
    val promotion_details = spark.sql(
      """
          values
          ('a1', 2, 7.0),
          ('a2', 1, 7.0), ('a2', 1, 9.0),
    ...
    val result1 = DataModelCheck.relationship(
      src = order_details,
      ref = promotion_details,
      keys = Seq("order_id", "article_id"),
      join = Seq("article_id", "promotion_id"),
    ...
    assertDataFramesEqual(result1, expected1)

    val result2 = DataModelCheck.relationship(
      src = order_details,
      ref = promotion_details,
      keys = Seq("order_id", "article_id"),
      join = Seq("article_id", "promotion_id"),
    ...
    assertDataFramesEqual(result2, expected2)

    val result3 = DataModelCheck.relationship(
      src = order_details,
      ref = promotion_details,
      keys = Seq("order_id", "article_id"),
      join = Seq("article_id", "promotion_id"),
    ...
    assertDataFramesEqual(result3, expected3)

    val result4 = DataModelCheck.relationship(
      src = order_details,
      ref = promotion_details,
      keys = Seq("order_id", "article_id"),
      join = Seq("article_id", "promotion_id"),
    ...
    assertDataFramesEqual(result4, expected4)
  }
  ...
  test("denormalization") {
    ...
    val result = DataModelCheck.denormalization(
      src = promotion_details,
      group = Seq("article_id", "supplier_id"),
      values = Seq("category_name", "supplier_phone"),
    )
  ...
  test("consistency") {
    ...
    val result = DataModelCheck.consistency(
      src = order_details,
      ref = promotion_details,
      join = Seq("article_id", "promotion_id"),
      values = Seq("category_name", "supplier_phone"),
  ...
  test("accuracy") {
    ...
    val result1 = DataModelCheck.accuracy(
      src = real_result,
      ref = expected_result,
      keys = Seq("order_date", "category_id"),
      exact = Seq("order_month", "order_counts"),
      approx = Map("sales_total" -> "1e-12", "sales_avg" -> "0.01"),
    )
    ...
    val result2 = DataModelCheck.accuracy(
      src = real_result,
      ref = expected_result,
      keys = Seq("order_date", "category_id")
    )
  ...
  test("accuracySorted") {
    ...
    val result1 = DataModelCheck.accuracySorted(
      src = real_result,
      ref = expected_result,
      group = Seq("order_date", "category_id"),
      sort = Seq("order_counts", "sales_total"),
      exact = Seq("order_month", "order_counts"),
      approx = Map("sales_total" -> "1e-12", "sales_avg" -> "0.01"),
    )
    ...
    val result2 = DataModelCheck.accuracySorted(
      src = real_result,
      ref = expected_result,
      group = Seq("order_date", "category_id"),
      sort = Seq("order_counts", "sales_total"),
    ...
    val result3 = DataModelCheck.accuracySorted(
      src = real_result,
      ref = expected_result,
      sort = Seq("order_counts", "sales_total"),
      exact = Seq("order_month", "order_counts"),
